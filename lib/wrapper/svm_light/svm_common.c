...
/*            not be larger than the weight vector!               */
{
    ...
    for (f = ex->fvec; f; f = f->next) sum += f->factor * sprod_ns(model->lin_weights, f);
    return (sum - model->b);
}
...
/* calculate the kernel function between two vectors */
{
    ...
    switch (kernel_parm->kernel_type) {
        case 0: /* linear */
            return (sprod_ss(a, b));
        case 1: /* polynomial */
            return (pow(kernel_parm->coef_lin * sprod_ss(a, b) + kernel_parm->coef_const,
...
void free_svector(SVECTOR *vec) {
    if (vec) {
        free(vec->words);
        if (vec->userdefined) free(vec->userdefined);
...
/* tests two sparse vectors for equality */
{
    ...
    while (ai->wnum && bj->wnum) {
        if (ai->wnum > bj->wnum) {
            if ((bj->weight) != 0) return (0);
            bj++;
        } else if (ai->wnum < bj->wnum) {
            if ((ai->weight) != 0) return (0);
            ai++;
        } else {
            if ((ai->weight) != (bj->weight)) return (0);
            ai++;
            bj++;
        }
    }
...
DOC *create_example(long docnum, long queryid, long slackid, double costfactor, SVECTOR *fvec) {
    DOC *example;
...
void free_example(DOC *example, long deep) {
    if (example) {
        if (deep) {
            if (example->fvec) free_svector(example->fvec);
        }
        free(example);
    }
}
...
void write_model(char *modelfile, MODEL *model) {
    ...
    sv_num = 1;
    for (i = 1; i < model->sv_num; i++) {
        for (v = model->supvec[i]->fvec; v; v = v->next) sv_num++;
    }
...
MODEL *read_model(char *modelfile) {
    ...
    for (i = 1; i < model->sv_num; i++) {
        ...
        if (!parse_document(line, words, &(model->alpha[i]), &queryid, &slackid, &costfactor, &wpos,
                            max_words, &comment)) {
...
MODEL *copy_model(MODEL *model) {
    ...
    for (i = 1; i < model->sv_num; i++) {
        ...
        newmodel->supvec[i] =
            create_example(model->supvec[i]->docnum, model->supvec[i]->queryid, 0,
    ...
    if (model->lin_weights) {
        ...
        for (i = 0; i < model->totwords + 1; i++) newmodel->lin_weights[i] = model->lin_weights[i];
    }
    return (newmodel);
}
...
void read_documents(char *docfile, DOC ***docs, double **label, long int *totwords, long int *totdoc) {
    char *line, *comment;
    WORD *words;
    ...
    while ((!feof(docfl)) && fgets(line, (int)ll, docfl)) {
        ...
        if (!parse_document(line, words, &doc_label, &queryid, &slackid, &costfactor, &wpos, max_words_doc,
                            &comment)) {
        ...
        (*docs)[dnum] =
            create_example(dnum, queryid, slackid, costfactor, create_svector(words, comment, 1.0));
...
int parse_document(char *line, WORD *words, double *label, long *queryid, long *slackid, double *costfactor,
