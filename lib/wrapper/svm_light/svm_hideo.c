...
int optimize_hildreth_despo(long, long, double, double, double, long, long, long, double, double *, double *,
                            double *, double *, double *, double *, double *, double *, double *, long *,
                            double *, double *);
int solve_dual(long, long, double, double, long, double *, double *, double *, double *, double *, double *,
               double *, double *, double *, double *, double *, double *, double *, long);
...
double *optimize_qp(qp, epsilon_crit, nx, threshold, learn_parm) QP *qp;
double *epsilon_crit;
...
/* only semi-definite. */
{
    ...
    if (!primal) { /* allocate memory at first call */
        ...
        buffer =
            (double *)my_malloc(sizeof(double) * ((nx + 1) * 2 * (nx + 1) * 2 + nx * nx + 2 * (nx + 1) * 2 +
                                                  2 * nx + 1 + 2 * nx + nx + nx + nx * nx));
        (*threshold) = 0;
        for (i = 0; i < nx; i++) {
            primal[i] = 0;
        }
    ...
    result = optimize_hildreth_despo(
        qp->opt_n, qp->opt_m, opt_precision, (*epsilon_crit), learn_parm->epsilon_a, maxiter,
        /* (long)PRIMAL_OPTIMAL, */
        (long)0, (long)0, lindep_sensitivity, qp->opt_g, qp->opt_g0, qp->opt_ce, qp->opt_ce0, qp->opt_low,
        qp->opt_up, primal, qp->opt_xinit, dual, nonoptimal, buffer, &progress);
    if (verbosity >= 3) {
    ...
    if ((result != PRIMAL_OPTIMAL) || (!(roundnumber % 31)) || (progress <= 0)) {
        ...
        result = optimize_hildreth_despo(
            qp->opt_n, qp->opt_m, opt_precision, (*epsilon_crit), learn_parm->epsilon_a, (long)maxiter,
            (long)PRIMAL_OPTIMAL, (long)SMALLROUND, lindep_sensitivity, qp->opt_g, qp->opt_g0, qp->opt_ce,
            qp->opt_ce0, qp->opt_low, qp->opt_up, primal, qp->opt_xinit, dual, nonoptimal, buffer, &progress);
        if (verbosity >= 3) {
            printf("return_srd(%d)...", result);
        }

        if (result != PRIMAL_OPTIMAL) {
            if (result != ONLY_ONE_VARIABLE) precision_violations++;
            if (result == MAXITER_EXCEEDED) maxiter += 100;
            if (result == NAN_SOLUTION) {
                lindep_sensitivity *= 2; /* throw out linear dependent examples more */
                                         /* generously */
...
int optimize_hildreth_despo(n, m, precision, epsilon_crit, epsilon_a, maxiter, goal, smallround,
                            lindep_sensitivity, g, g0, ce, ce0, low, up, primal, init, dual, lin_dependent,
                            buffer, progress) long n; /* number of variables */
                     ...
                     before and after */
{
    ...
    temp = &(buffer[n + (n + m) * 2 * (n + m) * 2 + (n + m) * 2 + n + m + n * n + (n + m) * 2 + n + n + n +
                    n * n]);
    ...
    for (i = 0; i < n; i++) { /* get variables with steepest feasible descent */
        sum = g0[i];
        for (j = 0; j < n; j++) sum += init[j] * g[i * n + j];
        sum = sum * ce[i];
        if (((b1 == -1) || (sum < bmin)) && (!((init[i] <= (low[i] + epsilon_a)) && (ce[i] < 0.0))) &&
            (!((init[i] >= (up[i] - epsilon_a)) && (ce[i] > 0.0)))) {
            bmin = sum;
            b1 = i;
        }
        if (((b2 == -1) || (sum >= bmax)) && (!((init[i] <= (low[i] + epsilon_a)) && (ce[i] > 0.0))) &&
            (!((init[i] >= (up[i] - epsilon_a)) && (ce[i] < 0.0)))) {
            bmax = sum;
            b2 = i;
        }
    ...
    if ((b1 != b2) && (m == 1)) {
        ...
        for (i = 0; i < n; i++) {
            if ((i != b1) && (i != b2)) {
                for (j = 0; j < n; j++) {
                    if (j == b1) g0_b1 += start[i] * g[i * n + j];
                    if (j == b2) g0_b2 += start[i] * g[i * n + j];
                }
                ce0_b -= (start[i] * ce[i]);
            }
        }
        if ((g[b1 * n + b2] == g[b1 * n + b1]) && (g[b1 * n + b2] == g[b2 * n + b2])) {
            ...
            if (ce[b1] == ce[b2]) {
                ...
                t = ((ce[b2] / ce[b1]) * g0[b1] - g0[b2] +
                     ce0[0] * (g[b1 * n + b1] * ce[b2] / ce[b1] - g[b1 * n + b2] / ce[b1])) /
                        ((ce[b2] * ce[b2] / (ce[b1] * ce[b1])) * g[b1 * n + b1] + g[b2 * n + b2] -
    ...
    if ((m > 0) &&
        ((fabs(g[b1 * n + b1]) < lindep_sensitivity) || (fabs(g[b2 * n + b2]) < lindep_sensitivity))) {
        /* printf("Case 0\n"); */
        add += 0.093274;
    }
    /* in case both examples are linear dependent */
    else if ((m > 0) && (g[b1 * n + b2] != 0 && g[b2 * n + b2] != 0) &&
             (fabs(g[b1 * n + b1] / g[b1 * n + b2] - g[b1 * n + b2] / g[b2 * n + b2]) < lindep_sensitivity)) {
        /* printf("Case lindep\n"); */
        add += 0.078274;
    }
    ...
    if ((m == 0) && (b1 >= 0)) {
        if (fabs(g[b1 * n + b1]) < lindep_sensitivity) {
            ...
            if (g0_b1 < 0) start[b1] = up[b1];
            if (g0_b1 >= 0) start[b1] = low[b1];
        }
    }
    if ((m == 0) && (b2 >= 0)) {
        if (fabs(g[b2 * n + b2]) < lindep_sensitivity) {
            ...
            if (g0_b2 < 0) start[b2] = up[b2];
            if (g0_b2 >= 0) start[b2] = low[b2];
        }
    }
    ...
    if ((!changed) || (n_indep > 1)) {
        result = solve_dual(n_indep, m, precision, epsilon_crit, maxiter, g_new, g0_new, ce_new, ce0_new,
                            low_new, up_new, primal, d, d0, ig, dual, dual_old, temp, goal);
    } else {
    ...
    if (verbosity >= 3) {
        printf("before(%.30f)...after(%.30f)...result_sd(%d)...", obj_before, obj_after, result);
    }

    return ((int)result);
}

int solve_dual(n, m, precision, epsilon_crit, maxiter, g, g0, ce, ce0, low, up, primal, d, d0, ig, dual,
               dual_old, temp, goal)
...
long goal;
{
    ...
    if ((m < 0) || (m > 1)) perror("SOLVE DUAL: inappropriate number of eq-constrains!");
...
void linvert_matrix(matrix, depth, inverse, lindep_sensitivity, lin_dependent) double *matrix;
long depth;
...
double calculate_qp_objective(opt_n, opt_g, opt_g0, alpha) long opt_n;
