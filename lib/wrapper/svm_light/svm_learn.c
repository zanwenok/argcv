...
void svm_learn_classification(DOC **docs, double *class, long int totdoc, long int totwords,
                ...
                optimization if not NULL. Array must be of size totdoc. */
{
    ...
    if (alpha) {
        ...
        update_linear_component(docs, label, index2dnum, alpha, a, index2dnum, totdoc, totwords, kernel_parm,
                                kernel_cache, lin, aicache, weights);
        (void)calculate_svm_model(docs, label, unlabeled, lin, alpha, a, c, learn_parm, index2dnum,
                                  index2dnum, model);
    ...
    /* train the svm */
    iterations = optimize_to_convergence(docs, label, totdoc, totwords, learn_parm, kernel_parm, kernel_cache,
    ...
    if (verbosity >= 1) {
        ...
        if (verbosity >= 2) {
            printf(
                "Runtime in cpu-seconds: %.2f (%.2f%% for kernel/%.2f%% for optimizer/%.2f%% for "
                "final/%.2f%% for update/%.2f%% for model/%.2f%% for check/%.2f%% for select)\n",
        ...
        if ((verbosity >= 1) && (!learn_parm->skip_final_opt_check)) {
            ...
            if ((!learn_parm->remove_inconsistent) && (!transduction)) {
                ...
                compute_xa_estimates(model, label, unlabeled, totdoc, docs, lin, a, kernel_parm, learn_parm,
                                     &(model->xa_error), &(model->xa_recall), &(model->xa_precision));
                if (verbosity >= 1) {
                    printf("done\n");
                }
    ...
    if (learn_parm->compute_loo) {
        ...
        for (heldout = 0; (heldout < totdoc); heldout++) {
            if (learn_parm->rho * a_fullset[heldout] * r_delta_sq + xi_fullset[heldout] < 1.0) {
                ...
                optimize_to_convergence(docs, label, totdoc, totwords, learn_parm, kernel_parm, kernel_cache,
                                        &shrink_state, model, inconsistent, unlabeled, a, lin, c,
        ...
        model->loo_precision =
            (trainpos - loo_count_pos) / (double)(trainpos - loo_count_pos + loo_count_neg) * 100.0;
        if (verbosity >= 1) {
...
void svm_learn_regression(DOC **docs, double *value, long int totdoc, long int totwords,
                          LEARN_PARM *learn_parm, KERNEL_PARM *kernel_parm, KERNEL_CACHE **kernel_cache,
                          MODEL *model)
/* docs:        Training vectors (x-part) */
...
/* model:       Returns learning result (assumed empty before called) */
{
    ...
    /* train the svm */
    iterations = optimize_to_convergence(docs, label, totdoc, totwords, learn_parm, kernel_parm,
                                         *kernel_cache, &shrink_state, model, inconsistent, unlabeled, a, lin,
                                         c, &timing_profile, &maxdiff, (long)-1, (long)1);
        ...
        if ((verbosity >= 1) && (!learn_parm->skip_final_opt_check)) {
            ...
            for (i = 0; i < totdoc; i++) {
                if ((lin[i] - model->b) * (double)label[i] <
                    (-learn_parm->eps + (double)label[i] * c[i]) - learn_parm->epsilon_crit)
                    loss +=
                        -learn_parm->eps + (double)label[i] * c[i] - (lin[i] - model->b) * (double)label[i];
                model_length += a[i] * label[i] * lin[i];
            }
            model_length = sqrt(model_length);
            fprintf(stdout, "L1 loss: loss=%.5f\n", loss);
    ...
    shrink_state_cleanup(&shrink_state);
    for (i = 0; i < totdoc; i++) free_example(docs[i], 0);
    free(docs);
    free(label);
    free(inconsistent);
    free(unlabeled);
...
void svm_learn_ranking(DOC **docs, double *rankvalue, long int totdoc, long int totwords,
                       LEARN_PARM *learn_parm, KERNEL_PARM *kernel_parm, KERNEL_CACHE **kernel_cache,
                       MODEL *model)
/* docs:        Training vectors (x-part) */
...
/* model:       Returns learning result (assumed empty before called) */
{
        ...
        for (j = i + 1; j < totdoc; j++) {
            if (docs[i]->queryid == docs[j]->queryid) {
                ...
                if (rankvalue[i] > rankvalue[j]) {
                    if (kernel_parm->kernel_type == LINEAR)
                        docdiff[k] = create_example(k, 0, 0, cost, sub_ss(docs[i]->fvec, docs[j]->fvec));
                    else {
    ...
    svm_learn_classification(docdiff, target, totpair, totwords, learn_parm, kernel_parm, (*kernel_cache),
                             pairmodel, NULL);
    ...
    for (i = 0; i < totpair; i++) free_example(docdiff[i], 1);
    free(docdiff);
...
void svm_learn_optimization(DOC **docs, double *rhs, long int totdoc, long int totwords,
                            LEARN_PARM *learn_parm, KERNEL_PARM *kernel_parm, KERNEL_CACHE *kernel_cache,
                            MODEL *model, double *alpha)
                ...
                optimization if not NULL. Array must be of size totdoc. */
{
    ...
    if (learn_parm->sharedslack)
        iterations = optimize_to_convergence_sharedslack(docs, label, totdoc, totwords, learn_parm,
                                                         kernel_parm, kernel_cache, &shrink_state, model, a,
                                                         lin, c, &timing_profile, &maxdiff);
    else
        iterations = optimize_to_convergence(docs, label, totdoc, totwords, learn_parm, kernel_parm,
                                             kernel_cache, &shrink_state, model, inconsistent, unlabeled, a,
                                             lin, c, &timing_profile, &maxdiff, (long)-1, (long)1);
    ...
    if (learn_parm->sharedslack) {
        ...
        for (i = 0; i < totdoc; i++) { /* create full index */
            index[i] = 1;
            if (maxslackid < docs[i]->slackid) maxslackid = docs[i]->slackid;
        }
        (void)compute_index(index, totdoc, index2dnum);
        ...
        for (i = 0; i <= maxslackid; i++) { /* create full index */
            loss += slack[i];
            if (alphaslack[i] > (learn_parm->svm_c - learn_parm->epsilon_a)) model->at_upper_bound++;
            if (alphaslack[i] > learn_parm->epsilon_a) svsetnum++;
        }
        free(index);
        free(index2dnum);
        free(slack);
...
long optimize_to_convergence(DOC **docs, long int *label, long int totdoc, long int totwords,
                             LEARN_PARM *learn_parm, KERNEL_PARM *kernel_parm, KERNEL_CACHE *kernel_cache,
                             SHRINK_STATE *shrink_state, MODEL *model, long int *inconsistent,
                             long int *unlabeled, double *a, double *lin, double *c, TIMING *timing_profile,
...
/* retrain: selects training mode (1=regular / 2=holdout) */
{
    ...
    for (; retrain && (!terminate); iteration++) {
        ...
        if (retrain == 2) {
            ...
            if (iteration % 101) {
                already_chosen = 0;
                if ((minl(learn_parm->svm_newvarsinqp, learn_parm->svm_maxqpsize - choosenum) >= 4) &&
                    ...
                    already_chosen = select_next_qp_subproblem_grad(
                        ...
                        learn_parm, inconsistent, active2dnum, working2dnum, selcrit, selexam, kernel_cache,
                        1, key, chosen);
                    choosenum += already_chosen;
                }
                choosenum += select_next_qp_subproblem_grad(
                    label, unlabeled, a, lin, c, totdoc,
                    minl(learn_parm->svm_maxqpsize - choosenum, learn_parm->svm_newvarsinqp - already_chosen),
                    learn_parm, inconsistent, active2dnum, working2dnum, selcrit, selexam, kernel_cache, 0,
                    key, chosen);
                ...
                choosenum += select_next_qp_subproblem_rand(
                    label, unlabeled, a, lin, c, totdoc,
                    minl(learn_parm->svm_maxqpsize - choosenum, learn_parm->svm_newvarsinqp), learn_parm,
                    inconsistent, active2dnum, working2dnum, selcrit, selexam, kernel_cache, key, chosen,
                    iteration);
            }
        }
        ...
        if (retrain != 2) {
            optimize_svm(docs, label, unlabeled, inconsistent, 0.0, chosen, active2dnum, model, totdoc,
                         working2dnum, choosenum, a, lin, c, learn_parm, aicache, kernel_parm, &qp,
                         &epsilon_crit_org);
        }

        if (verbosity >= 2) t3 = get_runtime();
        update_linear_component(docs, label, active2dnum, a, a_old, working2dnum, totdoc, totwords,
                                kernel_parm, kernel_cache, lin, aicache, weights);

        if (verbosity >= 2) t4 = get_runtime();
        supvecnum = calculate_svm_model(docs, label, unlabeled, lin, a, a_old, c, learn_parm, working2dnum,
                                        active2dnum, model);
        ...
        retrain = check_optimality(model, label, unlabeled, a, lin, c, totdoc, learn_parm, maxdiff,
                                   epsilon_crit_org, &misclassified, inconsistent, active2dnum,
        ...
        if ((!retrain) && (inactivenum > 0) &&
            ...
            t1 = get_runtime();
            reactivate_inactive_examples(label, unlabeled, a, shrink_state, lin, c, totdoc, totwords,
        ...
        if ((!retrain) && (transduction)) {
            ...
            if (verbosity == 1) printf("done\n");
            retrain =
                incorporate_unlabeled_examples(model, label, inconsistent, unlabeled, a, lin, totdoc, selcrit,
            ...
            activenum = shrink_problem(
                docs, learn_parm, shrink_state, kernel_parm, active2dnum, last_suboptimal_at, iteration,
                totdoc, maxl((long)(activenum / 10), maxl((long)(totdoc / 500), 100)), a, inconsistent);
            inactivenum = totdoc - activenum;
            if ((kernel_cache) && (supvecnum > kernel_cache->max_elems) &&
                ((kernel_cache->activenum - activenum) > maxl((long)(activenum / 10), 500))) {
                kernel_cache_shrink(kernel_cache, totdoc, minl((kernel_cache->activenum - activenum),
        ...
        if ((!retrain) && learn_parm->remove_inconsistent) {
            ...
            if (learn_parm->remove_inconsistent == 1) {
                retrain = identify_inconsistent(a, label, unlabeled, totdoc, learn_parm, &inconsistentnum,
                                                inconsistent);
                ...
                retrain = identify_one_misclassified(lin, label, unlabeled, totdoc, model, &inconsistentnum,
                                                     inconsistent);
            }
            if (retrain) {
...
long optimize_to_convergence_sharedslack(DOC **docs, long int *label, long int totdoc, long int totwords,
...
/* maxdiff: returns maximum violation of KT-conditions */
{
    ...
    for (; retrain && (!terminate); iteration++) {
        ...
        eq_target = 0;
        if (iteration % 101) {
            slackset = select_next_qp_slackset(docs, label, a, lin, slack, alphaslack, c, learn_parm,
                                               active2dnum, &maxsharedviol);
            if ((iteration % 2) || (!slackset) || (maxsharedviol < learn_parm->epsilon_crit)) {
                ...
                choosenum = select_next_qp_subproblem_grad(
                    label, unlabeled, a, lin, c, totdoc, learn_parm->svm_maxqpsize, learn_parm, ignore,
                    active2dnum, working2dnum, selcrit, selexam, kernel_cache, (long)0, key, chosen);
                learn_parm->biased_hyperplane = 0;
            }
        } else { /* once in a while, select a somewhat random working set
                    to get unlocked of infinite loops due to numerical
        ...
        optimize_svm(docs, label, unlabeled, ignore, eq_target, chosen, active2dnum, model, totdoc,
                     working2dnum, choosenum, a, lin, c, learn_parm, aicache, kernel_parm, &qp,
                     &epsilon_crit_org);
        learn_parm->biased_hyperplane = 0;
        ...
        retrain = check_optimality_sharedslack(docs, model, label, a, lin, c, slack, alphaslack, totdoc,
                                               learn_parm, maxdiff, epsilon_crit_org, &misclassified,
...
double compute_objective_function(double *a, double *lin, double *c, double eps, long int *label,
...
void optimize_svm(DOC **docs, long int *label, long int *unlabeled, long int *exclude_from_eq_const,
                  double eq_target, long int *chosen, long int *active2dnum, MODEL *model, long int totdoc,
                  long int *working2dnum, long int varnum, double *a, double *lin, double *c,
                  LEARN_PARM *learn_parm, CFLOAT *aicache, KERNEL_PARM *kernel_parm, QP *qp,
...
/* Do optimization on the working set. */
{
    ...
    compute_matrices_for_optimization(docs, label, unlabeled, exclude_from_eq_const, eq_target, chosen,
...
void compute_matrices_for_optimization(DOC **docs, long int *label, long int *unlabeled,
                                       long *exclude_from_eq_const, double eq_target, long int *chosen,
                                       long int *active2dnum, long int *key, MODEL *model, double *a,
                                       double *lin, double *c, long int varnum, long int totdoc,
                                       LEARN_PARM *learn_parm, CFLOAT *aicache, KERNEL_PARM *kernel_parm,
                                       QP *qp) {
    ...
    if (verbosity >= 3) {
        fprintf(stdout,
                "Computing qp-matrices (type %ld kernel [degree %ld, rbf_gamma %f, coef_lin %f, coef_const "
                "%f])...",
                kernel_parm->kernel_type, kernel_parm->poly_degree, kernel_parm->rbf_gamma,
                kernel_parm->coef_lin, kernel_parm->coef_const);
        fflush(stdout);
    }
    ...
    for (j = 1; j < model->sv_num; j++) { /* start at 1 */
        if ((!chosen[(model->supvec[j])->docnum]) && (!exclude_from_eq_const[(model->supvec[j])->docnum])) {
    ...
    for (i = 0; i < varnum; i++) {
        ...
        qp->opt_g0[i] =
            (learn_parm->eps - (double)label[key[i]] * c[key[i]]) + qp->opt_g0[i] * (double)label[key[i]];
    }
...
long calculate_svm_model(DOC **docs, long int *label, long int *unlabeled, double *lin, double *a,
                         double *a_old, double *c, LEARN_PARM *learn_parm, long int *working2dnum,
...
/* of alpha. */
{
    ...
    for (ii = 0; (i = working2dnum[ii]) >= 0; ii++) {
        ...
        if ((!b_calculated) && (a[i] > learn_parm->epsilon_a) && (a[i] < ex_c)) { /* calculate b */
    ...
    /* No alpha in the working set not at bounds, so b was not
       ...
       case. */
    if (learn_parm->biased_hyperplane && (!b_calculated) && (model->sv_num - 1 == model->at_upper_bound)) {
        first_low = 1;
        first_high = 1;
        b_low = 0;
        b_high = 0;
...
long check_optimality(MODEL *model, long int *label, long int *unlabeled, double *a, double *lin, double *c,
                      long int totdoc, LEARN_PARM *learn_parm, double *maxdiff, double epsilon_crit_org,
...
long check_optimality_sharedslack(DOC **docs, MODEL *model, long int *label, double *a, double *lin,
...
/* Check KT-conditions */
{
    ...
    for (ii = 0; (i = active2dnum[ii]) >= 0; ii++) {
        ...
        } else if ((a[i] >= ex_c) && (slack[docs[i]->slackid] < learn_parm->epsilon_shrink)) {
            last_suboptimal_at[docs[i]->slackid] = iteration; /* not likely optimal */
        }
    }
    /* termination criterion */
...
void compute_shared_slacks(DOC **docs, long int *label, double *a, double *lin, double *c,
                           long int *active2dnum, LEARN_PARM *learn_parm, double *slack, double *alphaslack)
/* compute the value of shared slacks and the joint alphas */
{
    ...
    for (jj = 0; (i = active2dnum[jj]) >= 0; jj++) { /* recompute slack variables */
        ...
        target = -(learn_parm->eps - (double)label[i] * c[i]);
        if ((target - dist) > slack[docs[i]->slackid]) slack[docs[i]->slackid] = target - dist;
        alphaslack[docs[i]->slackid] += a[i];
    }
}

long identify_inconsistent(double *a, long int *label, long int *unlabeled, long int totdoc,
                           LEARN_PARM *learn_parm, long int *inconsistentnum, long int *inconsistent) {
    long i, retrain;
...
long identify_misclassified(double *lin, long int *label, long int *unlabeled, long int totdoc, MODEL *model,
...
long identify_one_misclassified(double *lin, long int *label, long int *unlabeled, long int totdoc,
                                MODEL *model, long int *inconsistentnum, long int *inconsistent) {
...
void update_linear_component(DOC **docs, long int *label, long int *active2dnum, double *a, double *a_old,
                             long int *working2dnum, long int totdoc, long int totwords,
                             KERNEL_PARM *kernel_parm, KERNEL_CACHE *kernel_cache, double *lin,
...
/* in the current working set */
{
    ...
    if (kernel_parm->kernel_type == 0) { /* special linear case */
        ...
        for (jj = 0; (j = active2dnum[jj]) >= 0; jj++) {
            for (f = docs[j]->fvec; f; f = f->next) lin[j] += f->factor * sprod_ns(weights, f);
        }
    } else { /* general case */
        for (jj = 0; (i = working2dnum[jj]) >= 0; jj++) {
            if (a[i] != a_old[i]) {
                get_kernel_row(kernel_cache, docs, i, totdoc, active2dnum, aicache, kernel_parm);
...
long incorporate_unlabeled_examples(MODEL *model, long int *label, long int *inconsistent,
                                    long int *unlabeled, double *a, double *lin, long int totdoc,
                                    double *selcrit, long int *select, long int *key, long int transductcycle,
    ...
    if (transductcycle == 0) {
            ...
            if (unlabeled[i]) {
                if (label[i] == 1) {
                    learn_parm->svm_cost[i] =
                        learn_parm->svm_c * learn_parm->svm_costratio_unlab * learn_parm->svm_unlabbound;
                } else if (label[i] == -1) {
        ...
        if (verbosity >= 1) printf("Retraining.");
        if (verbosity >= 2) printf("\n");
        return ((long)3);
    }
    if ((transductcycle % check_every) == 0) {
        ...
        if ((!unsupaddnum1) && (!unsupaddnum2)) {
            if ((learn_parm->svm_unlabbound >= 1) && ((newpos + newneg) == allunlab)) {
                ...
                if (verbosity >= 1) printf("Number of switches: %ld\n", switchnum);
                return ((long)0);
            }
            ...
            printf("%ld positive -> Switching labels of %ld POS / %ld NEG unlabeled examples.", upos,
                   unsupaddnum1, unsupaddnum2);
            fflush(stdout);
        }
...
long select_next_qp_subproblem_grad(long int *label, long int *unlabeled, double *a, double *lin, double *c,
 ...
 those for which the kernel evaluations are cached. */
{
    ...
    for (i = 0; (j = active2dnum[i]) >= 0; i++) {
        ...
        if (valid && (!((a[j] <= (0 + learn_parm->epsilon_a)) && (s < 0))) &&
            (!((a[j] >= (learn_parm->svm_cost[j] - learn_parm->epsilon_a)) && (s > 0))) && (!chosen[j]) &&
            (label[j]) && (!inconsistent[j])) {
            selcrit[activedoc] =
                (double)label[j] * (learn_parm->eps - (double)label[j] * c[j] + (double)label[j] * lin[j]);
...
long select_next_qp_subproblem_rand(long int *label, long int *unlabeled, double *a, double *lin, double *c,
                                    long int totdoc, long int qp_size, LEARN_PARM *learn_parm,
                                    long int *inconsistent, long int *active2dnum, long int *working2dnum,
                                    double *selcrit, long int *select, KERNEL_CACHE *kernel_cache,
...
long select_next_qp_slackset(DOC **docs, long int *label, double *a, double *lin, double *slack,
                             double *alphaslack, double *c, LEARN_PARM *learn_parm, long int *active2dnum,
                             double *maxviol)
...
void init_shrink_state(SHRINK_STATE *shrink_state, long int totdoc, long int maxhistory) {
    long i;
...
void shrink_state_cleanup(SHRINK_STATE *shrink_state) {
    ...
    if (shrink_state->deactnum > 0) free(shrink_state->a_history[shrink_state->deactnum - 1]);
    free(shrink_state->a_history);
    free(shrink_state->last_a);
    free(shrink_state->last_lin);
}

long shrink_problem(DOC **docs, LEARN_PARM *learn_parm, SHRINK_STATE *shrink_state, KERNEL_PARM *kernel_parm,
                    long int *active2dnum, long int *last_suboptimal_at, long int iteration, long int totdoc,
                    long int minshrink, double *a, long int *inconsistent)
   ...
   minshrink variables can be removed. */
{
    ...
    for (ii = 0; active2dnum[ii] >= 0; ii++) {
        ...
        if (((iteration - lastiter) > learn_parm->svm_iter_to_shrink) || (inconsistent[i])) {
            change++;
        }
    }
...
void reactivate_inactive_examples(long int *label, long int *unlabeled, double *a, SHRINK_STATE *shrink_state,
...
/* Computes lin for those variables from scratch. */
{
    ...
    if (kernel_parm->kernel_type == LINEAR) { /* special linear case */
        ...
        for (t = shrink_state->deactnum - 1; (t >= 0) && shrink_state->a_history[t]; t--) {
            ...
            for (i = 0; i < totdoc; i++) {
                inactive[i] = ((!shrink_state->active[i]) && (shrink_state->inactive_since[i] == t));
...
void get_kernel_row(KERNEL_CACHE *kernel_cache, DOC **docs, long int docnum, long int totdoc,
                    long int *active2dnum, CFLOAT *buffer, KERNEL_PARM *kernel_parm)
...
void cache_kernel_row(KERNEL_CACHE *kernel_cache, DOC **docs, long int m, KERNEL_PARM *kernel_parm)
...
void cache_multiple_kernel_rows(KERNEL_CACHE *kernel_cache, DOC **docs, long int *key, long int varnum,
...
void kernel_cache_shrink(KERNEL_CACHE *kernel_cache, long int totdoc, long int numshrink, long int *after)
...
void kernel_cache_reset_lru(KERNEL_CACHE *kernel_cache) {
    ...
    for (k = 0; k < kernel_cache->max_elems; k++) {
        if (maxlru < kernel_cache->lru[k]) maxlru = kernel_cache->lru[k];
    }
    for (k = 0; k < kernel_cache->max_elems; k++) {
...
CFLOAT *kernel_cache_clean_and_malloc(KERNEL_CACHE *kernel_cache, long int docnum)
   ...
   is removed. */
{
    ...
    return ((CFLOAT *)((long)kernel_cache->buffer +
                       (kernel_cache->activenum * sizeof(CFLOAT) * kernel_cache->index[docnum])));
}
...
void compute_xa_estimates(MODEL *model, long int *label, long int *unlabeled, long int totdoc, DOC **docs,
                          double *lin, double *a, KERNEL_PARM *kernel_parm, LEARN_PARM *learn_parm,
...
double distribute_alpha_t_greedily(long int *sv2dnum, long int svnum, DOC **docs, double *a, long int docnum,
                                   long int *label, KERNEL_PARM *kernel_parm, LEARN_PARM *learn_parm,
                                   double thresh)
   ...
   computing a better bound using a greedy optimzation strategy. */
{
    ...
    for (d = 0; d < learn_parm->xa_depth; d++) {
        ...
        if (!allskip) {
            for (k = 0; k < svnum; k++) {
                cache[d * svnum + k] = kernel(kernel_parm, docs[sv2dnum[best_ex[d]]], docs[sv2dnum[k]]);
            }
        }
...
void estimate_transduction_quality(MODEL *model, long int *label, long int *unlabeled, long int totdoc,
                                   DOC **docs, double *lin)
   ...
   Dissertation. */
{
    ...
    printf("xacrit>=1: labeledpos=%.5f labeledneg=%.5f default=%.5f\n",
           (double)labpos / (double)totlab * 100.0, (double)labneg / (double)totlab * 100.0,
    ...
    printf("xacrit>=1: labeled=%.5f unlabled=%.5f all=%.5f\n", (double)lab / (double)totlab * 100.0,
           (double)ulab / (double)totulab * 100.0, (double)l / (double)(totdoc)*100.0);
    printf("xacritsum: labeled=%.5f unlabled=%.5f all=%.5f\n", (double)labsum / (double)totlab * 100.0,
           (double)ulabsum / (double)totulab * 100.0, (double)(labsum + ulabsum) / (double)(totdoc)*100.0);
...
{                             /* length of the longest support vector. This is */
    ...
    for (j = 1; j < model->sv_num; j++) {
        xlen =
            sqrt(kernel(kernel_parm, model->supvec[j], model->supvec[j]) -
                 2 * kernel(kernel_parm, model->supvec[j], nulldoc) + kernel(kernel_parm, nulldoc, nulldoc));
        if (xlen > maxxlen) {
            maxxlen = xlen;
        }
    }
...
double estimate_r_delta(DOC **docs, long int totdoc, KERNEL_PARM *kernel_parm) {
    ...
    maxxlen = 0;
    for (i = 0; i < totdoc; i++) {
        xlen = sqrt(kernel(kernel_parm, docs[i], docs[i]) - 2 * kernel(kernel_parm, docs[i], nulldoc) +
...
double estimate_r_delta_average(DOC **docs, long int totdoc, KERNEL_PARM *kernel_parm) {
    ...
    avgxlen = 0;
    for (i = 0; i < totdoc; i++) {
        avgxlen += sqrt(kernel(kernel_parm, docs[i], docs[i]) - 2 * kernel(kernel_parm, docs[i], nulldoc) +
                        kernel(kernel_parm, nulldoc, nulldoc));
    }
...
double length_of_longest_document_vector(DOC **docs, long int totdoc, KERNEL_PARM *kernel_parm) {
    long i;
    double maxxlen, xlen;
...
void write_prediction(char *predfile, MODEL *model, double *lin, double *a, long int *unlabeled,
                      long int *label, long int totdoc, LEARN_PARM *learn_parm) {
    FILE *predfl;
    long i;
    double dist, a_max;
